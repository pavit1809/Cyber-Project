...
int main()
{
	...
	string input;
	cout<<"Enter the string you want to get encrypted :";
	cin>>input;
	double n=input.size();
	int noofshifts=ceil(sqrt((double)n));
	//In encryption left shift	
	string finalinput="";
	for(int i=noofshifts;i<n;i++)
		finalinput+=input[i];
	...
	for(int i=0;i<n;i++)
	{
		int currpos=pos(finalinput[i]);
		currpos=data[currpos];
		output+=getchar(currpos);
	}
	cout<<"Your encrypted string is :"<<output;
}