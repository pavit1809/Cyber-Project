...
int main()
{
	...
	string input;
	cout<<"Enter the message you want to get encrypted(spaces as underscore(_)):";
	cin>>input;
	int n=input.size();
	string even="",odd="";
	for(int i=0;i<n;i++)
	...
	for(int i=0;i<odd.size();i++)
	{
		...
		int newpos=(currpos+shifter)%63;
		char newchar=getchar(data[newpos]);
		odd[i]=newchar;
	}
	for(int i=0;i<even.size();i++)
	{
		int currpos=pos(even[i]);
		int newpos=modsub(currpos,shifter,63);
		char newchar=getchar(data[newpos]);
		even[i]=newchar;	
	}
	string output="";
