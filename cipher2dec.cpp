...
pair<int,int> search(int data[63][63][2],int row,int col)
{
	for(int i=0;i<63;i++)
	{
		for(int j=0;j<63;j++)
		{
			if (data[i][j][0]==row && data[i][j][1]==col)
				return {i,j};
		}
	}
	return {-1,-1};
}
int search1(int *A,int n,int flag)
{
	for(int i=0;i<n;i++)
	{
		if (A[i]==flag)
...
int main()
{
	...
	string input;
	cout<<"Please the text you want to get decrypted:";
	cin>>input;
	int n=input.size();
	if (n==1)
	{
		...
		{
			int pos=input[0]-'A';
			int act=search1(alpha,26,pos);
			char nch=(char)(65+act);	
			cout<<"Your decrypted string is :"<<nch;
		}
		...
		{
			int pos=input[0]-'0';
			int act=search1(nums,10,pos);
			char nch=(char)(48+act);
			cout<<"Your decrypted string is :"<<nch;
		}
		else if (input[0]=='#')//for space
		{
			cout<<"Your decrypted string is :"<<'_';
		}
		return 0;
	}
	string answer="";
	for(int i=0;i<n-(n%2);i+=2)
	{
		if (checkc(input[i]) && checkc(input[i+1]))
		{
			int row=input[i]-'A';
			int col=input[i+1]-'A';
			pair<int,int> rc=search(data,row,col);
			int nrow=rc.first;
			int ncol=rc.second;
			answer+=getchar(nrow);
			answer+=getchar(ncol);
